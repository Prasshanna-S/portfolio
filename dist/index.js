(() => {
  ...
  var Clock = class {
    constructor(selector3) {
      this.element = document.querySelector(selector3);
      if (!this.element) {
  ...
  var Marquee = class {
    ...
    animate() {
      ...
      this.animation = gsapWithCSS.to(this.wrapper, {
        ...
        repeat: -1,
        onRepeat: () => {
          gsapWithCSS.set(this.wrapper, { x: 0 });
          console.log("repeat");
        }
      });
    }
  ...
  var WordPlacer = class {
    static placeWords(grid2, words, protectedZone, maxWords = CONFIG.revealWordCount) {
      const rows = grid2.length, cols = grid2[0].length, placed = [], used = /* @__PURE__ */ new Set();
      ...
      for (const w of shuffled) {
        ...
        const result = this._tryPlaceWord(grid2, w.toUpperCase(), placed.length, rows, cols, protectedZone, occupied);
        if (result) {
          placed.push(result);
          used.add(w);
    ...
    static _tryPlaceWord(grid2, word, idx, rows, cols, zone, occupied) {
      const len = word.length, canH = len <= cols, canV = len <= rows;
      if (!canH && !canV) return null;
      const dirs = this._getDirections(canH, canV);
      for (const d of dirs) {
        const placement = this._attemptPlacement(grid2, word, idx, d, rows, cols, zone, occupied);
    ...
    static _attemptPlacement(grid2, word, idx, dir, rows, cols, zone, occupied) {
      ...
      while (tries < CONFIG.maxDirectionAttempts) {
        ...
        if (this._canPlaceWord(grid2, word, pos, zone, occupied)) {
          this._placeWordInGrid(grid2, word, pos, idx);
    ...
    static _canPlaceWord(grid2, word, pos, zone, occupied) {
      return pos.every(({ r, c }, i) => {
        if (r < 0 || r >= grid2.length || c < 0 || c >= grid2[0].length) return false;
    ...
    static _placeWordInGrid(grid2, word, pos, idx) {
      pos.forEach(({ r, c }, i) => {
        grid2[r][c] = { letter: word[i], wordKey: idx };
      });
    }
  };
  ...
  var Grid = class {
    constructor() {
      ...
      this.catIndex = 0;
      this._usedWords = /* @__PURE__ */ new Set();
    ...
    _createBlocks() {
      ...
      const grid2 = Array.from({ length: this._rows }, () => Array(this._cols).fill(null));
      ...
      this._currentPlacedWords = WordPlacer.placeWords(grid2, category.words, zone, CONFIG.revealWordCount);
      this._currentPlacedWords.forEach((w) => this._usedWords.add(w.word));
      this.blocks = [];
      const count = this.iconImages.length;
      for (let r = 0; r < this._rows; r++) {
        for (let c = 0; c < this._cols; c++) {
          const cell = grid2[r][c] || { letter: Utils.randomLetter(), wordKey: null };
    ...
    _runPulseIntro() {
      ...
      this.blocks.forEach((b) => {
        ...
        b.revealed = true;
        b.empty = false;
        b.isBlankSpace = false;
      });
      const centerX = this.cssW / 2, centerY = this.cssH / 2;
      const sorted = this.blocks.slice().sort(
        (a, b) => Utils.distance(a.x + a.width / 2, a.y + a.height / 2, centerX, centerY) - Utils.distance(b.x + b.width / 2, b.y + b.height / 2, centerX, centerY)
      );
      this.currentTimeline = gsapWithCSS.timeline({
    ...
    _animateBlankFade(blankBlocks, fadeIn, onComplete) {
      blankBlocks.forEach((b) => {
        b.bubbleAlpha = fadeIn ? 0 : 1;
        b.scale = fadeIn ? 0.8 : 1;
      });
      this.currentTimeline = gsapWithCSS.timeline({ onComplete });
    ...
    _getHighlightBlocks() {
      return this._currentPlacedWords.flatMap(
        (w) => w.positions.map((p) => this.blocks.find(
          (b) => Math.round(b.x / this._blockSize) === p.c && Math.round(b.y / this._blockSize) === p.r
        ))
      ).filter(Boolean);
    }
    _categoryTimeline(titleBlocks, titleLetters) {
      ...
      this.currentTimeline = gsapWithCSS.timeline({ onComplete: () => this._onCategoryRevealEnd() });
      ...
      this.currentTimeline.to(nonBlankTitle, {
        ...
        duration: CONFIG.flipDuration + 0.09,
        ease: "power2.inOut",
        stagger: { amount: 0.11 },
        onUpdate: () => this._drawOnce()
      }, 0.09);
      ...
      this.currentTimeline.to(highlight, {
        ...
        bubbleAlpha: 1,
        scale: CONFIG.popScale,
        duration: CONFIG.flipDuration + 0.12,
        ease: "power2.inOut",
        stagger: { amount: 0.12 },
        onUpdate: () => this._drawOnce()
      }, 0.12);
