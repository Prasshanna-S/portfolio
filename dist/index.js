(() => {
  ...
  var Block = class {
    ...
    draw(ctx) {
      ...
      const h = this.height;
      const radius = 6;
      if (!isTextSide && scaleX < 0) ctx.scale(-1, 1);
      if (isTextSide) {
  ...
  var Grid = class {
    constructor() {
      this.blocks = [];
      this.wordTimelineMap = /* @__PURE__ */ new Map();
      this.autoRevealTimeline = null;
      this.hoveredWordIds = /* @__PURE__ */ new Set();
    ...
    createBlocks() {
      ...
      const grid2 = Array.from({ length: rows }, () => Array(cols).fill(null));
      let wordId = 0;
      ...
      while (wordId < WORDS.length && attempts < maxAttempts) {
        ...
        if (canPlace) {
          for (let i = 0; i < wordLength; i++) {
            const r = direction === "horizontal" ? startRow : startRow + i;
            const c = direction === "horizontal" ? startCol + i : startCol;
            grid2[r][c] = { letter: word[i], wordId };
          }
          wordId++;
        }
        attempts++;
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          if (grid2[r][c] === null) {
            ...
            grid2[r][c] = { letter: randomLetter, wordId: null };
          }
        }
      }
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          const { letter, wordId: wordId2 } = grid2[r][c];
          ...
          const usedIcons = neighbors.map((n) => n?.iconIndex).filter((i) => i !== void 0);
          let iconIndex;
          do {
          ...
          grid2[r][c].iconIndex = iconIndex;
          const block = new Block(x, y, this.blockWidth, letter, wordId2);
          block.iconIndex = iconIndex;
          block.iconImage = this.loadedIcons[iconIndex];
    ...
    setupEvents() {
      this.mouseMoveHandler = (e) => {
        ...
        const block = this.blocks.find(
          (b) => mouseX >= b.x && mouseX <= b.x + b.width && mouseY >= b.y && mouseY <= b.y + b.height
        );
      ...
      gsapWithCSS.ticker.add(() => this.draw());
    }
    ...
    revealWord(wordId, fromHover = false) {
      ...
      const tl = gsapWithCSS.timeline({ onComplete: () => this.wordTimelineMap.delete(wordId) });
    ...
    destroy() {
      ...
      gsapWithCSS.ticker.remove(() => this.draw());
