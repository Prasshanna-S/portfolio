(() => {
  ...
  var Grid = class {
    ...
    createBlocks() {
      ...
      for (let r = 0; r < rows; r++) {
        ...
        while (c < cols) {
          ...
          for (let i = 0; i < word.length; i++) {
            grid2[r][c + i] = { letter: word[i], wordId };
          }
          c += word.length;
          grid2[r][c] = { letter: "", wordId: null };
          c++;
          wordId++;
        }
      }
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          const { letter, wordId: wordId2 } = grid2[r][c] || { letter: "", wordId: null };
    ...
    revealWord(wordId, fromHover = false) {
      ...
      const blocks = this.blocks.filter((b) => b.wordId === wordId && b.wordId !== null).sort((a, b) => a.x - b.x);
