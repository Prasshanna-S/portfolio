(() => {
  ...
  var Grid = class {
    ...
    setup() {
      ...
      this.blocks = [];
      this.createBlocks();
      gsapWithCSS.ticker.add(this._draw);
    ...
    createBlocks() {
      ...
      let wid = 0, tries = 0;
      while (wid < WORDS.length && tries < 1e3) {
        ...
        for (let i = 0; i < L; i++) {
          ...
          if (grid2[r][c] && grid2[r][c].letter !== w[i]) {
            ok = false;
            break;
          }
        }
        if (ok) {
          for (let i = 0; i < L; i++) {
            const r = dir === "h" ? r0 : r0 + i;
            const c = dir === "h" ? c0 + i : c0;
            grid2[r][c] = { letter: w[i], wordId: wid };
          }
          wid++;
        }
        tries++;
    ...
    startAutoReveal() {
      ...
      const question = category.title;
      const wordsToShow = shuffle3(category.words).slice(0, Math.min(category.words.length, 10));
      const wordIds = wordsToShow.map((w) => WORDS.indexOf(w)).filter((id) => id >= 0);
      ...
      const bsAll = shuffle3(this.blocks.filter((b) => wordIds.includes(b.wordId)));
      const bs = bsAll.filter((b) => {
        return b.x + b.width < qxMin - pad || b.x > qxMax + pad || b.y + b.height < qyMin - pad || b.y > qyMax + pad;
      });
      bs.forEach((b) => {
        b.isQuestion = false;
        b.rotationY = 0;
    ...
    destroy() {
      gsapWithCSS.ticker.remove(this._draw);
